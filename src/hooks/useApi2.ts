...
const useApi = () => {
  ...
  const connect = useCallback(async () => {
    ...
    _api.on('connected', () => {
      ...
      _api.isReady.then((_api) => dispatch({ type: 'CONNECT_SUCCESS', payload: null }));
    });
    _api.on('ready', () => dispatch({ type: 'CONNECT_SUCCESS', payload: null }));
  ...
  const loadAccounts = useCallback(async () => {
    ...
    try {
      ...
      console.error(e);
      dispatch({ type: 'KEYRING_ERROR', payload: null });
    }
