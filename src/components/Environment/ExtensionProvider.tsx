...
export const ExtensionProvider: FC<AccountProviderProps> = ({
  appName,
  authRequired = true,
  children
}) => {
  ...
  const {
    ...
    open: openUploadMatedata,
    status: uploadMatedataStatus
  } = useModal(false);
  ...
  const setActiveAccount = useCallback(async (address: string | InjectedAccount): Promise<void> => {
